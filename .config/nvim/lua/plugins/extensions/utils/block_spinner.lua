...
--- @field timer uv_timer_t | nil Timer used to update spinner animation
...
function VirtualTextBlockSpinner.new(opts)
	...
	local display_width = vim.fn.max(vim.iter(lines)
		:map(function(line)
	...
	local repetitions = pattern_width > 0 and math.ceil(display_width / pattern_width) or display_width
